...
import { connectAuthEmulator, createUserWithEmailAndPassword, getAuth, onAuthStateChanged } from 'firebase/auth'
...
const SignupScreen = ({ navigation }) => {
    ...
    const handleSignUp = async (userEmail, userPassword, userLastName, userFirstName) => {
        //put signup stuff here
        const auth = getAuth();
        connectFirestoreEmulator(db, 'exp:', "192.168.0.193:19000")
        await createUserWithEmailAndPassword(auth, userEmail, userPassword)
            .then((userCredential) => {
                // Signed in 
                const user = userCredential.user;
                // db.collection(id, "workerUsers")
                //     .doc(user.uid)
                //     .set(ref(db, 'userworkers/' + user),{
                //         email: user.email,
    ...
    const handleSignUp1 = async (userEmail, userPassword, userLastName, userFirstName) => {
        try {
            ...
            dispatch(setCurrentUser({ user: cUser }));
            console.log(cUser);
            Alert.alert("Done Created Account");
        } catch (err) {
            Alert.alert("There is something wrong!!!!", err.message);
        }
