...
const SignupScreen = ({ navigation }) => {
    ...
    const handlelogOut = () => {
       serUser(null)
       console.log('user hs been loged out')
    }
            ...
            >
                <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                    <View className='flex flex-col items-center justify-center my-10'>
                        <View className='py-4'>
                            {user ? <Text className='font-bold text-lg'>Welcome, {user.email}</Text>: <Text>no one</Text>}
                        </View>
                        ...
                        <View className='p-4'>
                            <TouchableOpacity className='px-4 py-2 bg-purple-800  rounded-md w-80 items-center' onPress={handlelogOut}>
                                <Text className="text-lg text-purple-200">Sign out</Text>
                            </TouchableOpacity>
                        </View>
                        <View className=''>
