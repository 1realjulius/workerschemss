    ...
    const handleSignUp = async() => {
        ...
        createUserWithEmailAndPassword(auth, userEmail, userPassword)
            .then(async (userCredential) => {
                ...
                // if (user) {
                //     const docRef = doc(db, "workerProfiles", user.uid );
                //     const payload = {
                //         email: user.email,
                ...
                // }else{
                //     Alert.alert("Database details was not created");
                // }

                const uName = user.displayName;
                const uFN = userFirstName;
                const uLN = userLastName;
                const uEmail = user.email;
                const uPhone = user.phoneNumber;
                const uID = user.uid;
                const cUser = { userID: uID, userDisplayName: uName,userFirstName: uFN, userLastName: uLN, userMail: uEmail, userPhone: uPhone };
                dispatch(setCurrentUser({ user: cUser }));
                console.log(user.email);
                Alert.alert("Awsome,welcome" + uFN);
            })
            .catch((error) => {
