import { View, Text, KeyboardAvoidingView, ScrollView, TouchableOpacity, TextInput, Image, Alert } from 'react-native'
...
const EditProfileScreen = () => {
    ...
    const updateDetails = async () => {
        ...
        const userEMail = setUEmail(user.user)
        const db = getFirestore();
        const docRef = doc(db, 'workerDetails', user.user);
        const payload = {
            FirstName: FName,
            LastName: lName,
            Email: userEMail,
        ...
        await setDoc(docRef, payload)
            ...
            .catch((error) => {
                ...
                Alert.alert("ohh shit!", error.message);
                // ..l
            });
    }
        ...
        <KeyboardAvoidingView style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', }} behavior="padding" enabled keyboardVerticalOffset={60}>
                ...
                <View className='p-4'>
                    ...
                    <View className='py-4'>
                        ...
                        <View className="items-center">
                            <View className="border-4 flex relative justify-center items-center w-[170px] h-[170px] border-gray-300 rounded-full my-4">
                                {/* There should be a statement where if there is a user logged in display the user img from data base otherwise show icon */}
                    ...
                    <View className='mt-4'>
                        <View className='border-2 border-gray-300 divide-y-2 divide-gray-300 rounded-lg'>
                            ...
                            <View className='bg-white border border-gray-200 p-2 rounded-b-lg h-12 leading-none justify-center'>
                                {user ? <Text className='text-lg'>{uEmail}</Text> : <Text className='text-lg'>your Email cannot be changed</Text>}
                                <Text className='text-lg'>{uEmail}</Text>
                            </View>
                        </View>
                    </View>
            ...
            <View className='mx-4 my-2'>
                <TouchableOpacity
                    ...
                    className='items-center py-3 bg-purple-400 rounded-lg'
                >
